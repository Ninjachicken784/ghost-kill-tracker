...
public class KillSession {
    ...
    public void start() {
        if (!running) {
            ...
            paused = false;
            if (pauseStartTime != -1) {
                pausedElapsed += System.currentTimeMillis() - pauseStartTime;
                pauseStartTime = -1;
            }
        }
    }
    ...
    public void addScav(int amount) {
        if (!running || paused) return;
        totalScav += amount;
        sessionScav += amount;
    }
    ...
    public int getTotalScav() { return totalScav; }
    ...
    public int getSessionScav() { return sessionScav; }
